...
templates_with_one_pic = ['1.svg', '3.svg', '4.svg', '9.svg', '10.svg']
templates_with_two_pics = ['2.svg','5.svg', '6.svg', '7.svg']
# templates_with_no_pic = ['11.svg', '12.svg']
test_temp = ['8.svg']
...
pics_dir_list = sorted(pics_dir_list, key=numericalSort)
...
def update_svg(template_name, pics_list):
	...
	for i in range(0, len(pics_list)):
		...
		if(i == 0):
			...
			data = data.replace('id="image' + str(i) + '" xlink:href=""', 'id="image'+str(i)+'" xlink:href="' + encoded_base64_data_uri + '"')
...
gen_svg(test_temp, pics_list_two)

# gen_pdf(templates_with_one_pic)
# gen_pdf(templates_with_two_pics)
# gen_pdf(templates_with_no_pic)

# merger = PdfFileMerger()

# pdfs = os.listdir('./pdfs')
# pdfs = sorted(pdfs, key=numericalSort)
